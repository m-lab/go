...
func paramValue(v reflect.Value) (bq.QueryParameterValue, error) {
	...
	case typeOfTime:
		...
		res.Value = bigquery.CivilTimeString(v.Interface().(civil.Time))
		return res, nil

	case typeOfDateTime:
		res.Value = bigquery.CivilDateTimeString(v.Interface().(civil.DateTime))
		return res, nil
	...
	case typeOfRat:
		res.Value = bigquery.NumericString(v.Interface().(*big.Rat))
		return res, nil
	}
	switch t.Kind() {
	case reflect.Slice:
